    ...
    <div className="min-h-screen bg-[#fafaf9] text-slate-900 font-sans">
      <header className="h-16 bg-white border-b border-slate-200 sticky top-0 z-20">
        <nav className="max-w-6xl mx-auto h-full px-5 flex items-center justify-between">
          <button onClick={() => navigate('/')} className="flex items-center gap-2 font-semibold tracking-tight text-lg">
            <span className="w-8 h-8 rounded-lg bg-slate-900 text-white flex items-center justify-center"><FileText size={16} /></span>
            resumn<span className="text-indigo-600">.</span>
          </button>

          <div className="flex items-center gap-2 sm:gap-4">
            <a
              href="https://mail.google.com/mail/?view=cm&fs=1&to=nahg9957@gmail.com&su=Resume%20Builder%20Feedback&body=Hi%2C%0A%0AI%20want%20to%20share%20a%20suggestion%20or%20report%20a%20problem%3A%0A"
              target="_blank"
              rel="noreferrer"
            ...
            <div className="w-8 h-8 rounded-full bg-slate-100 border border-slate-200 flex items-center justify-center text-slate-700 text-sm font-semibold">
              {user?.username?.charAt(0).toUpperCase() || 'U'}
            </div>
            <button onClick={logout} className="p-2 text-slate-400 hover:text-slate-900 transition-colors" title="Log out">
              <LogOut size={18} />
            </button>
          </div>
        </nav>
      ...
      <main className="max-w-6xl mx-auto px-5 py-10 lg:py-14">
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-5 mb-10">
          <div>
            <p className="text-sm text-slate-500 mb-2">Your workspace</p>
            <h1 className="text-3xl sm:text-4xl font-semibold tracking-tight text-slate-950">
              My resumes
            </h1>
            <p className="mt-2 text-sm text-slate-500">Create and manage your resumes in one place.</p>
          </div>
          <button
            onClick={handleCreateNew}
            disabled={isCreating}
            className="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg bg-slate-900 text-white text-sm font-semibold hover:bg-slate-800 transition-colors disabled:opacity-60"
          >
        ...
        {isLoading && resumes.length === 0 ? (
          ...
          <div className="bg-white border border-dashed border-slate-300 rounded-xl p-12 text-center">
            ...
            <button onClick={handleCreateNew} className="px-4 py-2.5 rounded-lg bg-slate-900 text-white text-sm font-semibold hover:bg-slate-800">
              Create resume
            </button>
          </div>
        ) : (
          <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
            <div className="hidden sm:grid grid-cols-[1fr_160px_120px] gap-4 px-5 py-3 bg-slate-50 border-b border-slate-200 text-xs font-semibold text-slate-400 uppercase tracking-wider">
              <span>Resume</span>
            ...
            {resumes.map((resume, index) => {
              ...
              return (
                <div key={resume._id} className={`grid grid-cols-1 sm:grid-cols-[1fr_160px_120px] gap-3 sm:gap-4 items-center px-5 py-4 ${index > 0 ? 'border-t border-slate-100' : ''} hover:bg-slate-50/70 transition-colors`}>
                  <div className="min-w-0 flex items-center gap-3">
                    <div className="w-9 h-9 rounded-lg border border-slate-200 bg-white flex items-center justify-center text-slate-500 shrink-0">
                      <FileText size={17} />
                    </div>
                    <div className="min-w-0">
                      {isEditingTitle ? (
                        <input
                          ...
                          className="w-full max-w-sm h-9 px-2 border border-slate-300 rounded-md outline-none focus:border-slate-900 text-sm"
                        />
                      ) : (
                      ...
                      <p className="sm:hidden mt-1 flex items-center gap-1.5 text-xs text-slate-400">
                        <Clock size={11} /> {new Date(resume.updatedAt).toLocaleDateString()}
                      </p>
                    </div>
                  </div>

                  <div className="hidden sm:flex items-center gap-1.5 text-xs text-slate-500">
                    <Clock size={13} /> {new Date(resume.updatedAt).toLocaleDateString()}
                  </div>
                  ...
                  <div className="flex items-center justify-end gap-2">
                    <button
                      ...
                      className="inline-flex items-center justify-center gap-1.5 px-3 py-2 rounded-md bg-slate-900 text-white text-xs font-semibold hover:bg-slate-800 disabled:opacity-60"
                    >
                      {isOpening ? <Loader2 size={14} className="animate-spin" /> : <FileEdit size={14} />}
