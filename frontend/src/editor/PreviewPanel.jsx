...
//     const marginMap = { compact: '30px 40px', standard: '40px 50px', spacious: '50px 65px' };

//     const currentFontSize = `${documentStyle?.fontSize || 10}pt`;
//     const currentFontFamily = documentStyle?.fontFamily === 'Helvetica' ? 'Helvetica, Arial, sans-serif' : '"Times New Roman", Times, serif';
//     const currentLineHeight = lineSpacingMap[documentStyle?.lineSpacing] || '1.25';
//     const currentPadding = marginMap[documentStyle?.margins] || '40px 50px';
...
//       }}>
//         <div className="resume-section" style={{ textAlign: 'center', marginBottom: '8px' }}>
//           <div style={{ fontSize: '2.2em', fontWeight: 'bold', color: '#000', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '6px', lineHeight: '1' }}>
//             {fullName}
//           </div>
//           <div style={{ fontSize: '0.95em', color: '#000', textAlign: 'center', lineHeight: '1.4', marginTop: '4px', marginBottom: '8px' }}>
...
//             <div key={section.key} className="resume-section" style={{ marginBottom: '8px' }}>
...
//                   <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', fontWeight: 'bold', color: '#000', fontSize: '1.05em' }}>
//                     <span>{edu.institution}</span>
//                     <span style={{ fontWeight: 'normal', fontSize: '0.95em', whiteSpace: 'nowrap' }}>{edu.location || ''}</span>
//                   </div>
//                   <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', fontStyle: 'italic', color: '#000' }}>
...
//                   </div>
//                   <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', fontStyle: 'italic', marginBottom: '2px', color: '#000' }}>
//                     <span>{exp.company}</span>
//                     {exp.location && <span>{exp.location}</span>}
//                   </div>
//                   {exp.description && (
//                     <ul style={{ margin: '2px 0 0 0', paddingLeft: '24px', listStyleType: 'none' }}>
//                       {(Array.isArray(exp.description) ? exp.description : [exp.description]).filter(Boolean).map((d, j) => (
//                         <li key={j} style={{ position: 'relative', marginBottom: '2px' }}>
//                            <span style={{ position: 'absolute', left: '-14px' }}>•</span>{d}
//                         </li>
//                       ))}
//                     </ul>
//                   )}
...
//                           <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', fontWeight: 'bold', color: '#000' }}>
//                             <span>{item.title || ''}</span>
//                             <span style={{ fontWeight: 'normal', whiteSpace: 'nowrap', fontSize: '0.95em' }}>{item.date || ''}</span>
//                           </div>
//                       )}
//                       {item.bulletPoints && (
//                         <ul style={{ margin: (item.title || item.date) ? '2px 0 0 0' : '0', paddingLeft: '24px', listStyleType: 'none' }}>
//                           {(Array.isArray(item.bulletPoints) ? item.bulletPoints : [item.bulletPoints]).filter(Boolean).map((b, bIdx) => (
//                             <li key={bIdx} style={{ position: 'relative', marginBottom: '2px' }}>
//                                 <span style={{ position: 'absolute', left: '-14px' }}>•</span>{b}
//                             </li>
//                           ))}
//                         </ul>
//                       )}
...
//                   <input type="number" min="8" max="18" value={documentStyle?.fontSize || 10} onChange={(e) => updateDocumentStyle({ fontSize: Number(e.target.value) })} className="bg-zinc-900 border border-zinc-700 text-xs font-medium text-zinc-300 rounded-lg px-2 py-1 w-16 outline-none hover:bg-zinc-800 transition-colors" />
//                 </div>
//                 <div className="flex items-center gap-2">
...
const DynamicTemplate = ({ data, documentStyle }) => {
    ...
    const lineSpacingMap = { tight: '1.15', standard: '1.25', loose: '1.4' };
    const marginMap = { compact: '28px 36px', standard: '36px 46px', spacious: '46px 56px' };

    const currentFontSize = `${documentStyle?.fontSize || 10}pt`;
    const currentFontFamily = documentStyle?.fontFamily === 'Helvetica' ? 'Helvetica, Arial, sans-serif' : '"Times New Roman", Times, serif';
    const currentLineHeight = lineSpacingMap[documentStyle?.lineSpacing] || '1.25';
    const currentPadding = marginMap[documentStyle?.margins] || '36px 46px';
    ...
    return (
      <div id="resume-render-container" style={{ 
        ...
        <div className="resume-section" style={{ textAlign: 'center', marginBottom: '8px' }}>
          <div style={{ fontSize: '2.2em', fontWeight: 'bold', color: '#000', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '24px' }}>
            {fullName}
          </div>
          <div style={{ fontSize: '0.95em', color: '#000', textAlign: 'center', lineHeight: '1.4', marginBottom: '8px' }}>
            {[personal.phone, personal.email, personal.linkedin, personal.github, personal.location].filter(Boolean).join('\u00A0\u00A0\u00A0|\u00A0\u00A0\u00A0')}
          </div>
        </div>
        ...
        {sections.map(section => {
          ...
          if (section.key === 'summary' && data.summary) return (
            <div key={section.key} className="resume-section" style={{ marginBottom: '6px' }}>
              <div style={titleStyle}>{section.title}</div>
              <div style={{ textAlign: 'justify', paddingLeft: '4px' }}>{data.summary}</div>
            </div>
          );
          ...
          if (section.key === 'experience' && data.experience?.length > 0) return (
            <div key={section.key} className="resume-section" style={{ marginBottom: '6px' }}>
              ...
              {data.experience.map((exp, i) => (
                <div key={i} style={{ marginBottom: '8px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', fontWeight: 'bold', color: '#000', fontSize: '1.05em' }}>
                    <span>{exp.position || exp.title || exp.jobTitle}</span>
                    <span style={{ fontWeight: 'normal', fontSize: '0.95em', whiteSpace: 'nowrap' }}>{formatDates(exp.startDate, exp.endDate)}</span>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', fontStyle: 'italic', marginBottom: '2px', color: '#000' }}>
                    <span>{exp.company}</span>
          ...
          if (section.key === 'projects' && data.projects?.length > 0) return (
            <div key={section.key} className="resume-section" style={{ marginBottom: '6px' }}>
              ...
              {data.projects.map((proj, i) => (
                <div key={i} style={{ marginBottom: '8px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <span style={{ fontWeight: 'bold', color: '#000', fontSize: '1.05em' }}>{proj.name || proj.title}</span>
                    {proj.date && <span style={{ fontSize: '0.95em' }}>{proj.date}</span>}
                  </div>
                  {proj.subtitle && (
                      <div style={{ fontStyle: 'italic', marginBottom: '2px', color: '#000' }}>{proj.subtitle}</div>
