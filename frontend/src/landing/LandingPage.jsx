            ...
            <div className="grid lg:grid-cols-[0.92fr_1.08fr] gap-14 lg:gap-16 items-center">
              <motion.div initial={{ opacity: 0, x: -18 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.45 }}>
                ...
                <h1 className="text-[45px] sm:text-6xl lg:text-[64px] font-bold tracking-[-0.045em] leading-[1.04] text-slate-950">
                  Paste the job description.<br /><span className="text-blue-600">Get your resume in a minute.</span>
                </h1>
                <p className="mt-6 max-w-xl text-[17px] leading-8 text-slate-600">Start with the job you want. Add your experience once, tailor it to the role, and download a clean resume ready to apply.</p>
                <div className="mt-8">
                  <button onClick={() => navigate('/signup')} className="inline-flex items-center justify-center gap-2 px-6 py-3.5 rounded-lg bg-blue-600 text-white font-semibold hover:bg-blue-700 transition-all shadow-[0_8px_24px_rgba(37,99,235,0.22)] hover:-translate-y-0.5">Build my resume <ArrowRight size={18} /></button>
                </div>
              ...
              <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55, delay: 0.12 }} className="relative">
                ...
                <div className="relative bg-white border border-blue-100 rounded-2xl shadow-[0_20px_60px_rgba(30,64,175,0.12)] overflow-hidden">
                  <div className="h-12 px-5 border-b border-slate-100 flex items-center justify-between"><div className="flex items-center gap-2 text-sm font-semibold text-slate-800"><BriefcaseBusiness size={16} className="text-blue-600" /> Job description</div><span className="text-[11px] text-slate-400">Example</span></div>
                  <div className="p-5">
                    <div className="rounded-xl bg-slate-50 border border-slate-200 p-4 text-[13px] leading-6 text-slate-600 min-h-[170px]"><p className="font-semibold text-slate-900 mb-1">Software Engineer</p><p>We are looking for a developer with experience in Java, Spring Boot, REST APIs, SQL and cloud services.</p><p className="mt-2">Build reliable services, work with the team, and improve existing applications.</p></div>
                    <div className="mt-5 flex items-center gap-4"><div className="h-10 flex-1 rounded-lg bg-blue-600/10 border border-blue-100 flex items-center px-3"><span className="text-[12px] text-blue-700 font-medium">Resume generated</span></div><div className="w-10 h-10 rounded-lg bg-blue-600 flex items-center justify-center text-white shadow-sm"><ArrowRight size={17} /></div></div>
                  </div>
                </div>
                <motion.div animate={{ y: [0, -5, 0] }} transition={{ duration: 4, repeat: Infinity, ease: 'easeInOut' }} className="absolute -bottom-7 -left-7 hidden sm:block bg-white border border-blue-100 rounded-xl shadow-lg p-3 w-52"><div className="flex items-center gap-3"><div className="w-9 h-9 rounded-lg bg-blue-50 flex items-center justify-center text-blue-600"><FileText size={17} /></div><div><p className="text-[11px] font-semibold text-slate-800">Your resume</p><p className="text-[10px] text-slate-400">Ready to download</p></div></div></motion.div>
              </motion.div>
            </div>
          </div>
        </section>
